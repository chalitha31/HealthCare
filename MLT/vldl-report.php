...
`registered_patients`.`mobile` AS `mobile`, `registered_patients`.`address` AS `address`, `bloodtest`.`id` AS `bloodtest_id`
 ...
 WHERE `patients_details`.`id` = '" . $pdi . "'  ORDER BY `bloodtest`.`id` DESC LIMIT 1");
    ...
    <div class="container" id="form-container">
        <h2 class="report-title">VLDL Cholesterol</h2>
        ...
        <div class="additional-info">
            <strong>Note:</strong>
            <p>A VLDL (Very Low-density Lipoproteins) cholesterol test uses a blood sample. VLDL cholesterol is not directly measured, instead, the amount of VLDL cholesterol is estimated based on triglycerides results taken during a triglycerides test or a lipid panel test.</p>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            dataFeedButton.addEventListener('click', function() {
                document.getElementById('vldlCholesterol').value = (70 + Math.random() * 30 + 70).toFixed(2); // Glucose Fasting 70-100 mg/dL
            });
