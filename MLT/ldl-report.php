...
<head>
    ...
    <title>LDL Cholesterol Report</title>
...
`registered_patients`.`mobile` AS `mobile`, `registered_patients`.`address` AS `address`, `bloodtest`.`id` AS `bloodtest_id`
 ...
 WHERE `patients_details`.`id` = '" . $pdi . "'  ORDER BY `bloodtest`.`id` DESC LIMIT 1");
    ...
    <div class="container" id="form-container">
        <h2 class="report-title">LDL Cholesterol</h2>
        ...
        <div class="additional-info">
            <strong>Note:</strong>
            <p>LDL cholesterol test is a blood test that measures the level of low-density lipoprotein (LDL) cholesterol in the blood, which is commonly known as "bad cholesterol". LDL cholesterol is not directly measured, instead, the amount of LDL cholesterol is estimated based on HDL and VLDL results taken during a lipid panel test</p>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            dataFeedButton.addEventListener('click', function() {
                document.getElementById('vldlCholesterol').value = (70 + Math.random() * 30).toFixed(2); // Glucose Fasting 70-100 mg/dL
            });
